...
(
    ...
    `created_time` datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间（首次扫描时间）',
    `updated_time` datetime     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_url_method` (`url`,`method`) COMMENT 'URL+方法唯一（同一URL不同方法为不同接口）',
    KEY            `idx_status` (`status`) COMMENT '按状态筛选接口'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT '接口表（动态扫描维护）';
...
(
    ...
    `resource_type` enum('MENU','PAGE','BUTTON') NOT NULL COMMENT '资源类型：MENU-菜单、PAGE-页面、BUTTON-按钮',
    ...
    `visible`       tinyint               DEFAULT 1 COMMENT '是否可见（0-隐藏，1-显示，前端渲染用）',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT '资源表（手动维护：菜单、页面、按钮）';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT '用户角色关联表（用户删除时级联删除关联，角色删除时级联删除关联）';
...
(
    ...
    `created_by`   bigint            DEFAULT NULL COMMENT '创建人ID，关联sys_user表的id',
    `created_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '关联创建时间',
    PRIMARY KEY (`id`),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT '角色与资源的关联表（删除角色时级联删除关联，删除资源时级联删除关联）';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT '资源接口关联表（页面、按钮关联接口，资源/接口删除时级联删除关联）';
